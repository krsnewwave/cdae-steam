...
class RecommenderUtils():
    ...
    def get_sparse_content(self, df_content, pid_to_idx, include_eye=True):
        # get only the IDS that appear in the user content data
        df_content = df_content[df_content[self.itemId].isin(pid_to_idx.keys())].sort_values(by=self.itemId)

        # add new ids that are in user-content but not in content, then fill with 0's
        missing_ids = pd.Series(list(set(pid_to_idx.keys()).difference(set(df_content[self.itemId]))), name=self.itemId)
        df_content = df_content.append(missing_ids.to_frame())
        df_content = df_content.fillna(0)

        # sort the IDS in preparation to create indicator variables corresponding to the IDS
        df_content = df_content.reset_index().drop("index", axis=1)
        TAG_COLUMNS = [v for v in df_content.columns if v != self.itemId]
        sorted_ids = df_content[self.itemId].apply(lambda x : pid_to_idx[x])
        np_features = df_content.loc[sorted_ids][TAG_COLUMNS].values

        # create indicator variables and concatenate that with the features
        if include_eye:
            eye = sp.eye(np_features.shape[0], np_features.shape[0]).tocsr()
            item_features_concat = sp.hstack((eye, np_features))
            item_features_concat = item_features_concat.tocsr().astype(np.float32)
            return item_features_concat
        else:
            return sp.csr_matrix(np_features)
...
#    old code -- actually the mean of multiple precision@k (from 1 to k)
#    @staticmethod
#    def mapk(actual, predicted, k=10):
#        """
#        Computes the mean average precision at k.
...
#        return np.mean([RecommenderUtils.apk(actual, predicted, v) for v in range(1, k+1)])
